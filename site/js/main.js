...
(function () {
  ...
  (function () {
    ...
    if (!hash) return;
    var target;
    try { target = document.querySelector(hash + '.accordion-item'); } catch (e) { return; }
    if (!target) return;
  ...
  document.querySelectorAll('a[href^="#"]').forEach(function (anchor) {
    anchor.addEventListener('click', function (e) {
      var target;
      try { target = document.querySelector(this.getAttribute('href')); } catch (err) { return; }
      if (target) {
        e.preventDefault();
  ...
  // ── Contact form: pre-select from query string (?interest=…&role=…) ──
  (function () {
    var form = document.querySelector('form[name="contact"], form[name="contact-es"]');
    if (!form || !('URLSearchParams' in window)) return;
    var params = new URLSearchParams(window.location.search);
    ['interest', 'role'].forEach(function (key) {
  ...
  // ── Anchor-nav scroll spy (Resources, Glossary, Directory) ──
  (function () {
  ...
  // ── Back to top button (long pages only) ──
  (function () {
    ...
    btn.setAttribute('aria-label', isEs ? 'Volver arriba' : 'Back to top');
